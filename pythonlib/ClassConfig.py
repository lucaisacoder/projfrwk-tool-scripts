...
class ClassConfig:
    ...
    def __init__(self, project_path, config_filename, toolchain, toolchain_prefix):
        ...
        self.config_filename = self.config_path + "/" + config_filename
    ...
    def update(self):
        ...
        if self.toolchain.strip() != "":
            if not os.path.exists(self.toolchain):
                PRT.error("invalid toolchain path: %s" % self.toolchain)
                exit(1)
    ...
    def write(self):
        ...
        if self.update_config and self.config_content != self.config_content_old:
            ...
            if os.path.exists(self.config_path + "/build/global_config.mk"):
                os.remove(self.config_path + "/build/global_config.mk")
